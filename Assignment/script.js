async function fetchLatestLaunch() {
    try {
        ...
        document.getElementById('launch-name').textContent = `Launch Name: ${data.name || 'Unknown'}`;
        document.getElementById('launch-date').textContent = `Launch Date (UTC): ${data.date_utc || 'Unknown'}`;
        document.getElementById('rocket-name').textContent = `Rocket Name: ${rocketData.name || 'Unknown'}`;
        document.getElementById('launchpad-name').textContent = `Launchpad Name: ${launchpadData.name || 'Unknown'}`;
    } catch (error) {
...
async function fetchTotalLaunches() {
    try {
        const response = await fetch('https://api.spacexdata.com/v4/launches');
        const launches = await response.json();
        document.getElementById('total-launches').textContent = `Total SpaceX Launches: ${launches.length}`;
    } catch (error) {
